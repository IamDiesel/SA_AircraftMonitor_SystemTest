...
public final class AircraftFactory
{
	...
	{
		//System.out.println("AF"+msg);
		String entry [] = msg.split(";"); //msg is a ; seperated string generated by .toJedisString() method in one of the message (sub-)classes. AdsMessage generates at least: dataPath;mesageTypeD;originatorD ... so length must be at least 3
		if(entry.length < 3)
			throw new AircraftFactoryException(300,"String msg does not contain ennough parameters at message2Aircraft(). Size: "+entry.length+"/Min 3.",msg);
		return new Aircraft(Integer.parseInt(entry[2]),entry[0]); //entry 2 is always originatorD... which is the unique Aircraft Identifier
	}
	public void updateIdentification(String msg, Aircraft aircraft) throws AircraftFactoryException //when updateIdentification is called String msg is created by AirborneIdentificationMessage. Therby msg = messageTypeD;originatorD;SIXBIT_AircraftID
	{
		...
		if(entry.length < 4)
			throw new AircraftFactoryException(301,"String msg does not contain enough parameters at updateIdentification().Size: "+entry.length+"/Min 4",msg);
		aircraft.setFlightNo(entry[5]);
	}
	public void updateVelocity(String msg, Aircraft aircraft) throws AircraftFactoryException//when updateVelocity is called String msg is created by AirborneVelocityMessage. Therby msg = messageTypeD;originatorD;Airspeed;HeadingDegrees
	{
		String entry [] = msg.split(";");
		if(entry.length < 5)
			throw new AircraftFactoryException(302,"String msg does not contain enough parameters at updateVelocity().Size: "+entry.length+"/Min 5",msg);
		//System.out.println("Update"+msg);
		aircraft.setVelocity(Double.parseDouble(entry[5]));
		aircraft.setVeloAngle(Double.parseDouble(entry[6]));
	}
